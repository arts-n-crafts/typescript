...
Note: `AggregateRoot` is an _OOP_ pattern. The library also supports a purely
_functional_ alternative via [`Decider`](./Decider.md) — pure `decide()` and
...
export interface EventBasedAggregateRoot<
  TId,
