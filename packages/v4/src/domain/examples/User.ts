...
function evolveUserState(this: void, currentState: UserState, event: UserEvent): UserState {
  switch (event.type) {
    ...
    case "UserNameUpdated":
      return {
        ...currentState,
        name: (event as ReturnType<typeof createUserNameUpdatedEvent>).payload.name,
      };
    case "UserActivated":
...
function decideUserState(
  ...
  switch (command.type) {
    case "CreateUser": {
      ...
      return [
        createUserCreatedEvent(<string>command.aggregateId, command.payload, command.metadata),
      ];
    }
    case "UpdateUserName": {
      invariant(
        !isInitialState(currentState),
        fail(new UnexpectedUserState("expected mutated state during update")),
      );
      if (currentState.name === command.payload.name) {
        return [];
      }
      return [
        createUserNameUpdatedEvent(<string>command.aggregateId, command.payload, command.metadata),
      ];
    }
    case "ActivateUser": {
      invariant(
      ...
      return [
        createUserActivatedEvent(<string>command.aggregateId, command.payload, command.metadata),
      ];
    }
  }
}
