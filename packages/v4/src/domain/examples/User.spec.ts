...
describe("user decider", () => {
  ...
  const createCommand = createRegisterUserCommand(randomUUID(), {
    name: "Elon",
    email: "elon@x.com",
  });
  const updateUserName = createUpdateNameOfUserCommand(<string>createCommand.aggregateId, {
    name: "Donald",
  });
  const userCreated = createUserCreatedEvent(
    <string>createCommand.aggregateId,
    createCommand.payload,
  );
  const userNameUpdated = createUserNameUpdatedEvent(
    <string>createCommand.aggregateId,
  ...
  describe("user: create user", () => {
    it("should create the user", () => {
      pastEvents = [];
      const currentState = pastEvents.reduce(
        User.evolve,
        User.initialState(<string>createCommand.aggregateId),
      );
  ...
  describe("user: update name", () => {
    ...
    it("should not update the name of the user if the user does not exist", () => {
      ...
      expect(() => User.decide(command, currentState)).toThrow(
        "Invalid User state: expected mutated state during update.",
      );
    });
  });
