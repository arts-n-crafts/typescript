...
export class ResultedEventStore<TEvent extends DomainEvent> implements EventStore<
  TEvent,
  Promise<ResultedEventStoreAppendReturnType>,
  Promise<Result<TEvent[], Error>>
> {
  ...
  constructor(
    private readonly database: Database<
      ...
      Promise<Result<StoredEvent<TEvent>[], Error>>
    >,
  ...
  async append(streamName: string, events: TEvent[]): Promise<ResultedEventStoreAppendReturnType> {
    ...
    if (uniqueAggregateIds.size > 1) return Err(new MultipleAggregatesException());

    const event = events[0];
    const currentStream = await this.load(streamName, event.aggregateId);
    const eventsToStore = events.map((event) =>
      createStoredEvent(streamName, currentStream.unwrap().length + 1, event),
    );
    await Promise.all(
      eventsToStore.map(async (payload) =>
        this.database.execute(this.tableName, { operation: Operation.CREATE, payload }),
      ),
    );
