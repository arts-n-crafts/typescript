...
export interface ResultedRepositoryResult {
  id: string;
}

export class ResultedRepository<TState, TCommand, TEvent extends DomainEvent> implements Repository<
  TEvent,
...
> {
  ...
  async load(aggregateId: string): Promise<Result<TState, Error>> {
    ...
    return Ok(
      pastEventsResult.unwrap().reduce<TState>(this.evolveFn, this.initialState(aggregateId)),
    );
  }
