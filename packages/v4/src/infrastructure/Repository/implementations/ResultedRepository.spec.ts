...
describe("resulted repository", () => {
  ...
  let database: Database<
    StoredEvent<UserEvent>,
    Promise<ResultedEventStoreAppendReturnType>,
    Promise<Result<StoredEvent<UserEvent>[], Error>>
  >;
  let eventStore: EventStore<
    UserEvent,
    Promise<ResultedEventStoreAppendReturnType>,
    Promise<Result<UserEvent[], Error>>
  >;
  let repository: Repository<
    UserEvent,
    Promise<Result<UserState, Error>>,
    Promise<Result<ResultedRepositoryResult, Error>>
  >;
  ...
  it("should store the decider user event", async () => {
    ...
    const events = User.decide(createCommand, currentState) as UserEvent[];
