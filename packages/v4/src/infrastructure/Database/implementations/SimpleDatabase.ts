...
export class SimpleDatabase<TModel extends WithIdentifier> implements Database<
  TModel,
  Promise<void>,
  Promise<TModel[]>
> {
  ...
  async query(tableName: string, specification: CompositeSpecification<TModel>): Promise<TModel[]> {
    if (this.simulateOffline) throw new DatabaseOfflineException();

    const tableRecords = this.datasource.get(tableName) || [];
    return tableRecords.filter((record: TModel) => specification.isSatisfiedBy(record));
  }
