...
export class ResultedCommandBus<TCommand extends Command> implements CommandBus<
  TCommand,
  Promise<ResultedCommandBusExecutionReturnType>,
  Result<void, Error>
> {
  private handlers: Map<TCommand["type"], CommandHandler<TCommand>> = new Map();

  register(
    aTypeOfCommand: TCommand["type"],
    anHandler: CommandHandler<TCommand>,
  ): Result<void, Error> {
