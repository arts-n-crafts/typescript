...
describe("resulted event bus", () => {
  ...
  let eventBus: EventConsumer<
    UserEvent,
    EventHandler<UserCreatedEvent>,
    Promise<Result<void, Error>>,
    Result<void, never>
  > &
    EventProducer<UserEvent, Promise<Result<void, Error>>>;
  ...
  it("should be able publish events", async () => {
    ...
    const sentEventCausedByCreatedEventIndex = events.findIndex(
      (event) => event.metadata.causationId === createdEvent.id,
    );
