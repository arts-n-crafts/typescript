...
export class ResultedEventBus<TEvent extends BaseEvent>
  implements
    EventConsumer<TEvent, EventHandler<TEvent>, Promise<Result<void, Error>>, Result<void, never>>,
    EventProducer<TEvent, Promise<Result<void, Error>>>
{
  ...
  async consume(stream: string, anEvent: TEvent): Promise<Result<void, Error>> {
    if (!this.handlers.has(stream)) return Ok(undefined);
    ...
    const errs = results.reduce((previousValue, currentValue) => {
      ...
      if (err) previousValue.push(err);
      return previousValue;
    }, [] as Error[]);
    if (errs.length) return Err(new AggregateError(errs, "EventBus::consume > return errors"));
    return Ok(undefined);
  }

  subscribe(stream: string, aHandler: EventHandler<TEvent>): Result<void, never> {
