...
describe("simple query bus", () => {
  ...
  beforeEach(async () => {
    ...
    statement = {
      ...
      payload: { id: randomUUID(), name: "Elon", email: "elon@x.com", prospect: true },
    };
  ...
  it("should throw an error if the query handler is already registered", () => {
    ...
    expect(() => bus.register(query.type, handler)).toThrow(
      `Handler already registered for query type: ${query.type}`,
    );
  });
  ...
  it("should throw an error if the query handler is not registered", async () => {
    const query = createGetUserByEmailQuery(payload);
    await expect(bus.execute(query)).rejects.toThrowError(
