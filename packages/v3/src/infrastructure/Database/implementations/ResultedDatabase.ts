...
} from "./SimpleDatabase.exceptions.ts";

export type ResultedDatabaseExecuteReturnType = Result<{ id: string }, Error>;

export class ResultedDatabase<TModel extends WithIdentifier> implements Database<
  TModel,
...
> {
  ...
  async query(
    ...
    if (this.simulateOffline) return Err(new DatabaseOfflineException());

    const tableRecords = this.datasource.get(tableName) || [];
    return Ok(tableRecords.filter((record: TModel) => specification.isSatisfiedBy(record)));
  }

  async execute(
    tableName: string,
    statement:
      | CreateStatement<TModel>
      | PutStatement<TModel>
      | PatchStatement<TModel>
      | DeleteStatement,
  ): Promise<ResultedDatabaseExecuteReturnType> {
    if (this.simulateOffline) return Err(new DatabaseOfflineException());
    ...
    switch (statement.operation) {
      case Operation.CREATE: {
        ...
        if (isDuplicate) return Err(new DuplicateRecordException(statement.payload.id));
      ...
      case Operation.PUT: {
        ...
        if (index === -1) return Err(new RecordNotFoundException(statement.payload.id));
